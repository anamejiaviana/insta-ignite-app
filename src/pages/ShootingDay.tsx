...
export default function ShootingDay() {
  ...
  const sourcePlanId = locState?.sourcePlanId as string | undefined;
  const sourceContent = locState?.sourceContent as {
    title?: string;
    hook?: string;
    shots?: string[];
    script?: string;
  ...
  const generateFromCalendar = async () => {
    ...
    try {
      ...
      await saveShootingPlan({ planData: data, numDays, weeklyPlanId: latestPlan.id });
  ...
  const runCustomIdeaPlan = async (ideaValue: string) => {
    const trimmedIdea = ideaValue.trim();
    if (!trimmedIdea) {
    ...
    try {
      ...
      const normalizedPlanData = {
        ...
        reels: [
          {
            idea: sourceContent?.title || trimmedIdea,
            title: sourceContent?.title || trimmedIdea,
            hook: data.hook,
            storyboard: data.storyboard,
            shots: data.shots || sourceContent?.shots || [],
  ...
  const generateFromIdea = async () => {
    await runCustomIdeaPlan(customIdea);
  };

  useEffect(() => {
    if (!fromContent || !prefillIdea || !activeClient || autoGeneratedRef.current) return;
    autoGeneratedRef.current = true;
    setMode("custom");
    setCustomIdea(prefillIdea);
    void runCustomIdeaPlan(prefillIdea);
  }, [activeClient, fromContent, prefillIdea]);
